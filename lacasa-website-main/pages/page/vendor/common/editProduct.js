...
const EditProduct = ({ setToggleEditProduct }) => {
  ...
  const [productData, setProductData] = useState({ short_description: "" });
  ...
  const [editorState, setEditorState] = useState(
    EditorState.createWithContent(
      convertFromHTML(productData?.short_description)
    )
  );
  ...
  useEffect(() => {
    if (id)
      API.readAll(`/vendor/products/${id}?include=categories`)
        .then(async (response) => {
          const arr = [];
          ...
          for (let i = 0; i < response.data.images.length; i++) {
            img.push(response.data.images[i]);
          }
          const attribute = [];
  ...
  const handleProductImageDelete = async (e) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].id == e.target.id) {
        arr.splice(i, 1);
        await deleted.push(parseInt(e.target.id));
      }
    }
  ...
  const onSubmit = async (data) => {
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      if (tableData[i].key === "Fabric Color") {
    ...
    if (tableData.length === 0) {
      formData.append(`attributes`, tableData);
    }

    API.create(`/vendor/products/${router.query.id}`, formData)
      .then((response) => {
        if (response.data) {
          router.push({
            query: {
                            ...
                            {productImages.length > 0 && (
                              <Row>
                                {productImages.map((image, index) => (
                                  <Col key={index} sm="3">
                                    <div
                                      className={"position-relative mt-3 mb-4"}
                                    >
                                      <div
