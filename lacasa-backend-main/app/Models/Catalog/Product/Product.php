...
class Product extends Model implements TranslatableContract, Feedable
{
    use HasFactory, LogsActivity, Translatable, HasSlug;
    ...
    public function scopeSlug(Builder $query, $value): Builder
    {
        return $query->when(!is_numeric($value), function ($query) use ($value) {
            //  $string = $value;
            // $id = str_replace('-','',strrchr($value, '-'));
            // return $query->when(preg_match('/^[0-9]+$/',$id), function ($query) use ($value,$id) {
            //     $query->where('slug', '=', $value)->get();
        ...
        return $query->select('products.*')
            ->where(function ($query) use ($value) {
                $query->whereTranslationLike('name', '%' . $value . '%')
                    ...
                    ->orwhereRelation('attribute_values', 'value', 'like', '%' . $value . '%')
    ...
    public function getMetaKeywordsTranslateAttribute()
    {
        $langs = getLanguages();
        foreach ($langs as $k => $lang) {
            $data[$lang['locale']] = $this->hasTranslation($lang['locale']) ? $this->translate($lang['locale'])->meta_keywords : null;
        }
        return $data;
    }

    public function getMetaDescriptionTranslateAttribute()
    {
        $langs = getLanguages();
        foreach ($langs as $k => $lang) {
            $data[$lang['locale']] = $this->hasTranslation($lang['locale']) ? $this->translate($lang['locale'])->meta_description : null;
        }
        return $data;
    }

    public function getMetaTitleTranslateAttribute()
    {
        $langs = getLanguages();
        foreach ($langs as $k => $lang) {
            $data[$lang['locale']] = $this->hasTranslation($lang['locale']) ? $this->translate($lang['locale'])->meta_description : null;
    ...
    public function toFeedItem(): FeedItem
    {
        return FeedItem::create([
            ...
            "title" => $this->name,
            "price" => $this->price,
            "sale_price" => $this->price_after_sale,
            "availability" => $this->stock ? "in stock" : "out of stock",
            "image" => $this->images->first()->url,
            "summary" => $this->description,
            "updated" => $this->updated_at,
            "link" => config('app.front_url') . '/product-details/' . $this->slug,
            "authorName" => 'La Casa Egy',
        ]);
    ...
    {
        return Product::where('status', 'Published')->with('images')->get();
    }
}
